...
async def topup_saldo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    MENU TOPUP SALDO VIA QRIS PAKASIR
    Step:
    1. Tampilkan pilihan nominal
    2. User pilih nominal atau custom
    """
    if update.callback_query:
        query = update.callback_query
        await query.answer()
        
        # --- BUAT KEYBOARD NOMINAL ---
        keyboard = [
            [
            ...
            [
                InlineKeyboardButton("✏️ Input Nominal Lain", callback_data='topup_custom'),
            ],
        ...
        await query.edit_message_text(
            ...
            "Pilih nominal topup di bawah ini:\n"
            "Minimal: Rp 1.000\n"
            "Maksimal: Rp 1.000.000\n\n"
            "Atau pilih <b>✏️ Input Nominal Lain</b> untuk custom.",
            parse_mode='HTML',
            reply_markup=reply_markup
        )
    else:
        await update.message.reply_text(
            "💰 <b>TOP UP SALDO VIA QRIS</b>\n\n"
            "Silakan pilih nominal di menu.",
            parse_mode='HTML'
        )
    
    return TOPUP_NOMINAL
    
async def topup_pilih_nominal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    HANDLER PILIH NOMINAL TOPUP
    Step:
    1. User pilih nominal dari tombol
    2. Validasi nominal
    3. Proses ke fungsi proses_topup_qris
    """
    query = update.callback_query
    await query.answer()
    ...
    # --- AMBIL NOMINAL DARI CALLBACK ---
    try:
        amount = int(data.replace('topup_', ''))
    except:
        await query.edit_message_text("❌ Nominal tidak valid!")
        return PROTOKOL
    
    # --- VALIDASI ---
    if amount < 1000:
        await query.edit_message_text("❌ Minimal topup Rp 1.000")
        return TOPUP_NOMINAL
    if amount > 1000000:
        await query.edit_message_text("❌ Maksimal topup Rp 1.000.000")
        return TOPUP_NOMINAL
    ...
    # --- PROSES TOPUP ---
    return await proses_topup_qris(update, context, amount)
    
async def input_nominal_topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    INPUT NOMINAL CUSTOM DARI USER
    Step:
    1. User kirim angka
    2. Validasi
    3. Proses ke fungsi proses_topup_qris
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('topup_'):
        return await topup_pilih_nominal(update, context)
 ...
 # ==========================================
# FUNGSI TOPUP QRIS PAKASIR
# ==========================================

async def proses_topup_qris(update: Update, context: ContextTypes.DEFAULT_TYPE, amount: int):
    """
    PROSES TOPUP VIA QRIS PAKASIR
    Step:
    1. Buat invoice di Pakasir
    2. Generate QR Code dari payment_number
    3. Kirim QR Code ke user
    4. Mulai polling status pembayaran
    """
    user_id = update.effective_user.id
    ...
    # --- STEP 9: SIMPAN CONTEXT ---
    context.user_data['topup_order_id'] = order_id
    context.user_data['topup_amount'] = amount
    
    # --- STEP 10: MULAI POLLING (BACKGROUND) ---
    asyncio.create_task(start_polling_topup(update, context, order_id, user_id, amount))
    
    return PROTOKOL
    
async def start_polling_topup(update: Update, context: ContextTypes.DEFAULT_TYPE, 
                               order_id: str, user_id: int, amount: int):
    """
    POLLING STATUS PEMBAYARAN DI BACKGROUND
    Step:
    1. Polling ke Pakasir setiap interval
    2. Jika sukses → tambah saldo user
    3. Jika expired → kirim notifikasi
    """
    pakasir = get_pakasir()
    if pakasir is None:
        pakasir = init_pakasir(context.bot)
    
    # --- CALLBACK SAAT SUKSES ---
    async def on_complete(uid, amt, oid):
        """Dipanggil ketika pembayaran berhasil"""
        # Tambah saldo user
        update_balance(uid, amt, f"Topup via QRIS Pakasir (ID: {oid})")
        ...
        try:
            ...
            await context.bot.send_message(
                ...
                text=f"✅ <b>PEMBAYARAN BERHASIL!</b>\n\n"
                     ...
                     f"Gunakan /start untuk melanjutkan.",
                parse_mode='HTML'
            )
    ...
    # --- CALLBACK SAAT EXPIRED ---
    async def on_expired(uid, oid):
        """Dipanggil ketika pembayaran expired"""
        try:
            await context.bot.send_message(
                chat_id=uid,
                text=f"⏰ <b>PEMBAYARAN KADALUARSA</b>\n\n"
                     f"🆔 Order ID: <code>{oid}</code>\n"
                     f"Silakan lakukan topup ulang dengan menekan tombol Topup.",
                parse_mode='HTML'
            )
        except Exception as e:
            print(f"❌ Gagal kirim notifikasi expired ke {uid}: {e}")
    
    # --- MULAI POLLING ---
    await pakasir.start_polling(
        order_id=order_id,
        user_id=user_id,
...
async def cek_status_topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    CEK STATUS PEMBAYARAN MANUAL (DIPANGGIL USER)
    Step:
    1. Ambil order_id dari callback
    2. Cek status ke Pakasir
    3. Tampilkan hasil ke user
    """
    query = update.callback_query
    await query.answer()
    ...
    pakasir = get_pakasir()
    if pakasir is None:
        await query.edit_message_text("❌ Sistem sedang sibuk, coba lagi nanti.")
        return PROTOKOL
    ...
    # --- STEP 4: AMBIL DATA TRANSAKSI ---
    trx = pakasir.get_transaction(order_id)
    amount = trx.get('amount', 0) if trx else 0
    
    # --- STEP 5: TAMPILKAN HASIL ---
    await query.edit_message_text(
        f"{status_emoji.get(status, '❓')} <b>STATUS PEMBAYARAN</b>\n\n"
        f"🆔 Order ID: <code>{order_id}</code>\n"
        f"💰 Nominal: Rp {amount:,}\n"
    ...
    # --- STEP 6: JIKA SUKSES, TAMBAH SALDO (JIKA BELUM) ---
    if status == 'completed':
        # Cek apakah saldo sudah ditambahkan
        trx = pakasir.get_transaction(order_id)
        if trx and not trx.get('balance_added', False):
            update_balance(user_id, amount, f"Topup via QRIS Pakasir (ID: {order_id})")
            trx['balance_added'] = True
            # Simpan kembali
            from pakasir_core import save_pakasir_transaction
            save_pakasir_transaction(order_id, trx)
            
            await context.bot.send_message(
                chat_id=user_id,
                text=f"✅ <b>SALDO BERHASIL DITAMBAHKAN!</b>\n\n"
                     f"💰 Nominal: Rp {amount:,}\n"
                     f"💵 Saldo baru: Rp {get_balance(user_id):,}",
                parse_mode='HTML'
            )
...
async def batal_topup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Step:
    1. Ambil order_id dari callback
    2. Hapus transaksi dari database lokal
...
# ==========================================
# MAIN FUNCTION - DENGAN PAKASIR INTEGRATION
# ==========================================
def main():
    print("=" * 60)
    ...
    # ==========================================
    # 3. BUAT DIREKTORI TOPUP (JIKA PERLU)
    # ==========================================
    try:
    ...
    # ==========================================
    # 4. BUAT FILE VISITOR (JIKA BELUM ADA)
    # ==========================================
    try:
        visitor_file = "/etc/conf/visitors.txt"
    ...
    # ==========================================
    # 7. INISIALISASI PAKASIR PAYMENT GATEWAY
    # ==========================================
    try:
    ...
    # ==========================================
    # 8. TAMBAHKAN ERROR HANDLER
    # ==========================================
    app.add_error_handler(error_handler)
    print("✅ Error handler added")
    
    # ==========================================
    # 9. TAMBAHKAN COMMAND HANDLER
    # ==========================================
    ...
    # ==========================================
    # 10. TAMBAHKAN CALLBACK QUERY HANDLER UNTUK TOPUP
    # ==========================================
    ...
    print("✅ Topup callback handlers added")
    
    # ==========================================
    # 11. TAMBAHKAN CALLBACK QUERY HANDLER UNTUK PAKASIR
    # ==========================================
    ...
    app.add_handler(CallbackQueryHandler(topup_pilih_nominal, pattern='^topup_'))
    print("✅ Pakasir callback handlers added")
    
    # ==========================================
    # 12. TAMBAHKAN BUTTON HANDLER (UTAMA)
    # ==========================================
    app.add_handler(CallbackQueryHandler(button_handler))
    print("✅ Main button handler added")
    
    # ==========================================
    # 13. TAMBAHKAN CONVERSATION HANDLER
    # ==========================================
    conv_handler = ConversationHandler(
        entry_points=[CommandHandler('start', start)],
        states={
            # ==========================================
            # STATE PROTOKOL - PILIH LAYANAN
            # ==========================================
            PROTOKOL: [
                ...
                # TAMBAHKAN UNTUK PAKASIR
                CallbackQueryHandler(topup_pilih_nominal, pattern='^topup_'),
                CallbackQueryHandler(cek_status_topup, pattern='^cek_status_'),
                CallbackQueryHandler(batal_topup, pattern='^batal_topup_'),
            ],
            ...
            # ==========================================
            # STATE INPUT PASSWORD (KHUSUS SSH)
            # ==========================================
            INPUT_PASSWORD: [
            ...
            # ==========================================
            # STATE TOPUP NOMINAL (PAKASIR)
            # ==========================================
            TOPUP_NOMINAL: [
                # Pilihan nominal dari tombol
                CallbackQueryHandler(topup_pilih_nominal, pattern='^topup_'),
            ...
            # ==========================================
            # STATE TOPUP BUKTI (TIDAK DIPAKAI - LEGACY)
            # ==========================================
            TOPUP_BUKTI: [
            ...
            # ==========================================
            # STATE VOUCHER GENERATE MIN BALANCE
            # ==========================================
            VOUCHER_GENERATE_MIN_BALANCE: [
    ...
    # ==========================================
    # 14. TAMBAHKAN HANDLER UNTUK MENU LAINNYA
    # ==========================================
